...
> site.** All 210 pages are statically prerendered from source with no per-request data,
...
| `npm run build` | `prisma generate` then production build. Prerenders all 210 pages as static HTML |
...
- **Next.js 14, App Router, TypeScript, Tailwind.** 210 pages, all statically prerendered at build time. Only the four `/api/*` routes are dynamic.
