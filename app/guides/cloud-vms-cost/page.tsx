...
const faqs = [
  ...
  { question: 'Does Camzify publish a price?', answer: 'It starts from $5 per camera per month, and most cameras land between $20 and $90 per camera per month depending on detections and patrol rounds, with cloud storage priced per terabyte on top. The rest is quoted per site, lower for an annual term and for more features per camera, which is why the pricing page asks for your counts rather than listing rates. Camzify is priced per instance per month, a stream instance per camera plus a detection instance per AI feature per camera and cloud storage, and the quote depends on those counts, retention, and the number of sites and sub-users. Ask for one with your camera count and it comes back against your own guarding cost.' },
  ...
  { question: "How does the total cost of ownership compare with a recorder?", answer: "A recorder is bought once and then costs disks, replacement every few years, a login per box, someone to maintain it, and the footage that is lost when it fails or is stolen. A cloud VMS is a monthly figure per instance with no hardware, so the comparison is that figure over the recorder's life against the purchase, the disks, the replacements and the staff time. Put your own counts into the comparison on this page rather than a vendor's." },
];
