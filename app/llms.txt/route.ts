...
A patrol sequence is an ordered list of camera stops, each with its own checklist. Every checklist item is marked Compliant or Not Compliant. Marking an item Not Compliant captures a snapshot and offers a message to the guard assigned to that camera. An item cannot remain Not Compliant: before the round can close it must either be fixed and re-checked, capturing a second snapshot and marking it Fixed and verified, or be held as Pending with a written reason. Compliant and Fixed items count toward the compliance percentage; Pending items count against it. Automated rounds can judge a camera from a one, two or three second window of live video (scene observation) rather than a single frame. Every round produces a timestamped report, openable as a web report or a PDF, containing each camera checked, every item with its result, the snapshot the item was judged against, before and after frames for anything fixed during the round, the written reason on anything pending, the guard notified on any failure, and an overall compliance percentage.
