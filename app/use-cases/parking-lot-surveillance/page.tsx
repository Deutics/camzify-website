...
const faqs = [
  ...
  { question: 'What counts as an after-hours event?', answer: 'Whatever the zone rule says. A person in the lot between midnight and 5am can be the event on a staff car park and not on a residential one. Zones carry schedules, so the same lot can have different rules for the customer area and the staff area.' },
...
const content: UseCaseContent = {
  ...
  lede: <>
    ...
    A lot is large, unevenly lit and generates more footage than anyone will review, which makes it the most common blind spot on a commercial site. Camzify tracks people and vehicles as objects on the existing cameras, applies rules with hours, and checks the lot on a patrol round.
  </>,
  ...
  handles: {
    ...
    paras: [
      <><Link href="/ai-features/zone-intrusion-detection" className="text-primary hover:underline">Zone intrusion detection</Link> covers the person case: the lot, or parts of it, as zones with hours, so a tracked person walking between parked cars at 2am raises an alert with the snapshot. <Link href="/ai-features/illegal-parking-detection" className="text-primary hover:underline">Illegal parking detection</Link> covers the vehicle case: a fire lane, a disabled bay or a restricted area where a vehicle stopped beyond the allowed time is the event. <Link href="/ai-features/wrong-way-vehicle-detection" className="text-primary hover:underline">Wrong-way vehicle detection</Link> fires on a vehicle moving against a lane.</>,
    ...
    detections: [
      { href: '/ai-features/zone-intrusion-detection', name: 'Zone intrusion detection', role: 'The lot as zones with hours. A tracked person after hours is the event.' },
