 ...
 *
 * The two steps sit in one frame: the inputs and a live summary of what they add up to
 * on the left, the contact fields on the right, so it is plain that the request is for
 * exactly what was entered and the two columns stay the same height on a desktop.
 */
    ...
    <div id="quote" className="scroll-mt-28 overflow-hidden rounded-2xl border border-border bg-card">
      <div className="grid lg:grid-cols-2">
        <div className="p-6 sm:p-8 lg:border-r lg:border-border">
          <Step n={1} title="Build your configuration" sub="Counts, not commitments. Change any number and the summary follows." />
          <div className="mt-6">
        ...
        <div className="border-t border-border bg-background/40 p-6 sm:p-8 lg:border-t-0">
          <Step n={2} title="Get the quote for it" sub="Back within one business day, set for your site, with any annual-term or volume discount applied." />
          <div className="mt-6">
            <FormWrapper
              endpoint="/api/quote"
              ...
              {({ loading }: { loading: boolean }) => (
                <>
                  ...
                  <div>
                    ...
                    <textarea id="quote-message" name="message" rows={5} className="mt-1.5 w-full rounded-lg border border-border bg-background px-4 py-3 text-sm focus:border-primary focus:ring-1 focus:ring-primary" placeholder="Sites, camera brands, an annual term, a partner portfolio" />
                  </div>
                  ...
                  <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
                    The counts in step 1 are sent with your request. We store what you send so we can reply, and nothing else happens with it. See the{' '}
