...
// weaken this file to silence errors — fix the flagged code (see components/system/client-only.tsx
// and components/system/safe-format.tsx for ready-made safe patterns).
        ...
        return {
          Identifier(node) {
            ...
            if (!inFunction) {
              context.report({
                node,
                message: `\`${node.name}\` at module scope runs during SSR where it is undefined. Move it inside useEffect, an event handler, or wrap the UI in <ClientOnly> (components/system/client-only.tsx).`,
              })
            }
          },
        }
        ...
        return {
          CallExpression(node) {
            if (node.callee.type === 'MemberExpression' && !node.callee.computed && node.callee.property.type === 'Identifier' && LOCALE_METHODS.has(node.callee.property.name) && node.arguments.length === 0) {
              context.report({
                node,
                message: `\`${node.callee.property.name}()\` without arguments uses the runtime's locale/timezone, which differs between server and client (hydration mismatch). Pass an explicit locale (and timeZone for dates), e.g. ('en-US', { timeZone: 'UTC' }), or use <SafeDate>/<SafeTime>/<SafeNumber> from components/system/safe-format.tsx.`,
              })
            }
          },
        }
