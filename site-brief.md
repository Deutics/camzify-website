...
>
> Built from the site's own source (210 pages), not a crawl — so the service
...
- **What they actually sell (one sentence):** A per-instance, per-month subscription (a stream instance per camera, a detection instance per AI feature per camera, cloud storage in terabytes) that runs scheduled AI patrol rounds on the IP cameras a business already owns — checking a defined checklist at each camera, flagging failures, notifying the assigned guard, and producing a timestamped PDF compliance report.
- **Price positioning:** Mid-to-premium, but deliberately benchmarked against **manned guarding cost, not competing software**. Pricing is quote-based; no rate card is published. Verified 2026-09-16.
...
3. **Platform** (11 pages) — dashboard, live streaming, video backup and retention, notifications and alerts, analytics and reporting, user management, permission groups, license and instance management, multi-site management, mobile access, AI architecture.
...
- **WHO THE LEADS ACTUALLY ARE (from the business's outreach lead files, 35 files, analyzed 2026-09-05; aggregates only, no individuals recorded):** 12,794 companies, 16,398 people. By primary NACE code: **56% guarding companies (80.1)**, **34% security-systems installers, i.e. CCTV, alarm and access-control installers (80.2)**, ~5% electrical/installation contractors, <1% investigation. By country: **US 35%, Germany 30%, UK 14%**, France 4%, Italy 2.5%, Hungary 2%, then NL/ES/FI/SE/BE/IE/AT around 1% each; roughly 60% of the list is in Europe and about 45% is in non-English-speaking countries. By size: **87% have 1–50 staff** (guarding: half are 11–50; installers: half are 1–10); only 4% are over 200. Of the people with a job title, **71% are the owner, CEO, MD or Geschäftsführer**; 4% are operations, control-room or security managers. What they sell today, from their own descriptions: guarding companies mention mobile patrols (15%), manned guarding (9%), CCTV (8%) and alarm monitoring (4%); installers mention CCTV (53%), access control (31%), fire (17%) and monitoring (17%). **0% mention remote or virtual guarding**, so the category has to be introduced, not assumed. Implications: the buyer is an owner-operator of a small security business, not a corporate security manager; the pitch is revenue and margin, not cost-cutting; the comparison set is mobile patrols and alarm monitoring, not enterprise VMS; and the partner track (agencies, installers, monitoring companies) is the primary track, not a secondary one. See `docs/AUDIENCE-STRATEGY.md`.
...
- **Legal pages (privacy policy, terms of service, cookie policy, accessibility) were written 2026-09-07 from what the site actually does** (four forms stored in a database, no analytics or advertising scripts, a theme preference and two session flags in browser storage, Singapore governing law). They are drafts for counsel to review before the site leaves maintenance; the cookie policy lists every browser-storage item and must be updated before any analytics tool is added.
- **Pricing model, stated by the business 2026-09-16:** per instance per month. Every connected camera takes a **camera stream instance** (connects it, streams it live, makes it recordable and patrollable); every AI feature enabled on a camera takes a **detection instance of that feature** (100 cameras with line intrusion on 10 of them is 100 stream instances plus 10 line intrusion instances); **cloud storage** is sold per terabyte per month as a pool for the account, and the customer decides how it is spent: retention is set per camera in days or as a storage cap, or applied to a whole site, and may be longer on some cameras than on others (confirmed from the video backup screen, 2026-09-16). Each instance type has its own monthly price; an account's plan (the console labels it, e.g. "Enterprise Plan") is the set of instances and storage held for a term, not a published tier. Approximate list rates, published from 2026-09-16 and held only in `lib/pricing-estimates.ts`: stream instance about USD 6 per camera per month (motion and camera tampering detection included free), most detections under USD 20 per instance (the site uses 20 as a ceiling so estimates never undercut a quote), virtual patrolling about USD 35 per camera (manual and automated rounds), behavioral anomaly and weapons detection about USD 40, storage about USD 12 per TB per month. Quotes come in below list for an annual term and for more features per camera. With discounts a stream instance comes down to about USD 5. On 2026-09-17, after reviewing that the comparable software-only vendors (Solink, Spot AI, Coram, Pro-Vigil, Eagle Eye) publish no rate card, the business withdrew the public estimate: the list rates stay in the file for the team's lead emails only, and the one figure the site states is "from $5 per camera per month".
- **Pricing disclosure, decided by the business 2026-09-17:** one public figure, "from $5 per camera per month", and no other rate, range or estimate on the site. The pricing page and the demo form collect the reader's camera and feature counts as a quote request (no figure shown); the server adds a list-rate estimate to the team's lead email only. The ROI calculator shows the reader's own figures and no Camzify cost. Quotes are promised within one business day, the same commitment the demo form makes.
- **Pricing disclosure, revised 2026-09-19:** the business delegated the decision on a public range to the site editor, who chose to publish one band beside the floor, because the pages answer engines cite for cost queries all carry a per-camera range: "most cameras land between $20 and $90 per camera per month depending on which detections run and whether scheduled patrol rounds are on; cloud storage is priced separately per terabyte per month". Derived from the list rates (stream plus one or two detections, with or without patrol rounds). Held as `PUBLIC_TYPICAL_RANGE` in `lib/pricing-estimates.ts`. Nothing more precise is public.
- **Storage location, stated by the business 2026-09-19:** footage is written to cloud object storage (Amazon S3) in the AWS region nearest the customer's sites, so it stays retrievable and close to where it was recorded. State it that way; do not name a specific region or promise residency in a named country.
- **Certification timing, stated by the business 2026-09-19:** PDPA, GDPR, SOC 2 Type II and ISO 27001 are targeted for completion by the end of 2026. Still in progress and not held; the date may be stated as a target, never as a promise.
- **Why US customers left ADT, stated by the business 2026-09-19:** instability, false alarms and a hard-to-use, old-style interface. Rule 8 still applies: on the site these appear only as what the switchers said they were looking for (fewer false alarms, no recorder dependency, a usable interface), never as claims about ADT.
- **AI-feature schedule semantics, stated by the business 2026-09-04:** the schedule on an AI feature is a *notification window*. The detection runs regardless; notifications are generated only inside the window and none outside it. Never write it as "schedule-based activation" or "zones with hours" (the detection is not switched off). House phrasing: "a notification window per camera", "notified in the hours you set".
- **Partner facts confirmed by the business 2026-09-03:** Camzify sells no hardware. There is no certified installer program, no partner portal, no white-label option and no published partner margin; partner terms are agreed in conversation. A suspended sub-user keeps its sites, recordings and reports and can be reinstated. The five `/partners/*` pages state these; if any changes, change the page and this line together.
...
- **Humour:** None. Not one joke across 210 pages. Do not introduce any.
